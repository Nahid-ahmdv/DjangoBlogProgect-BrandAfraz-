...
class Category(models.Model):  #یه سلف ریلیشن با خودش داره صرفا
    ...
    title = models.CharField(max_length=200)  #وقتی این کلاس پرینت میشه چه چیزی رو نمایش بده یا معادلا یک اینستنسی از اون آبجکت اگه پرینت شد چه چیزی رو در رابطه با اون اینستنس نمایش بده
    # This method defines how an instance of the 'Category' model will be represented as a string. When you call str(instance) or when an instance is printed, this method will return the value of the title field. This is particularly useful for displaying category names in Django admin or other interfaces.
    def __str__(self):
        return self.title
...
class Comment(models.Model):  #اگر قابلیت ریپلای زدن به یک کامنت را هم بخواهیم فعال کنیم، باید به خودش فارن کی بزنیم و همون سلف ریلیشن خودمون میشه
    ...
    def __str__(self):
        return f"{self.text[:7]}..."  #تا هفتا کاراکتر اول رو ریترن میکنه